...
const snakeColor = [
  { fill: "#300047", stroke: "#300047" },
  { fill: "#b691ff", stroke: "#36117e" },
  { fill: "#002c57", stroke: "#003f7d" },
  { fill: "#bfcfff", stroke: "#002db3" },
];
let w = 100;
let h = 100;
let score = 0;
  ...
  for (let i = 0; i < snakePoint.length; i++) {
    for (let l = 0; l < snakePoint.length; l++) {
      if (i != l && snakePoint.length > 3) {
        if (
          snakePoint[i].x == snakePoint[l].x &&
          snakePoint[i].y == snakePoint[l].y
        ) {
...
const getScore = function () {
  if (!pseudoSay) {
    pseudo = prompt(
      "Vous avez réussi à collecter " + score + " point.\n Donnez votre pseudo !"
    );
  } else {
    return;
  }
...
const createHighScoreContent = function () {
  ...
  const highScoreBest = highScore.sort((a, b) =>
  ...
  for (let i = 0; i < highScoreLength; i++) {
    ...
    newValue =
      "<div class='highScoreElement'>" +
      (i + 1) +
      ". <span class='pseudo'>" +
      highScoreBest[i].pseudo +
      "</span> à eu " +
      highScoreBest[i].score +
      "</div>";
